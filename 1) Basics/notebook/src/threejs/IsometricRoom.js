...
import roomModel from 'assets/room.json'
...
let room = () => {
    ...
    var screen = new Mesh(
      new PlaneGeometry(.31,.25,.85),
      new MeshStandardMaterial({emissive: 0x141A35})
    );
    ...
    function light() {
      ...
      var spotlight = new SpotLight(0xF5FC5A);
      spotlight.position.set(1.75, 4, -3);
      spotlight.castShadow = true;
      spotlight.intensity = .2;
      scene.add(spotlight);
  
      var dirlight = new DirectionalLight(0xfdd8ff);
      dirlight.position.set(-.96,3,-.75);
      //dirlight.castShadow = true;
      dirlight.intensity = .2;
      scene.add(dirlight);
  
      var ambi = new AmbientLight(0x0e1642);
      scene.add(ambi);
    ...
    function Snow() {
      ...
      this.vel = new Vector3(-1 * (0.0005 + Math.random() * 0.001),-1 * (0.005 + Math.random() * 0.01), -.1 * (0.005 + Math.random() * 0.01));
    ...
    document.querySelector('canvas.webgl').append(renderer.domElement);
    renderer.render(scene, camera);
